...
if (
	...
	if ($date_now > $exp_date) {
		...
		$q = "UPDATE `user` SET `email_confirmation` = 1, `activated_at` = '$date_now' WHERE (`email` ='" . mysqli_real_escape_string($conn, $_GET['x']) . "' AND `token` ='" . mysqli_real_escape_string($conn, $_GET['y']) . "') LIMIT 1";

		$r = mysqli_query($conn, $q) or trigger_error("Query: $q\b<br/>MySQL Error: " . mysqli_error($conn));


		if (mysqli_affected_rows($conn) == 1) {
			$b = "<h5 class='text-success'>Your account has been activated! Please <a class='a-link text-success' href='login.php'>log in.</a></h5>";
			header("refresh:59;url=index.php");
		} else {
